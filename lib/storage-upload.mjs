...
export function mediaStorageEnabled() {
  return process.env.KOINOPS_MEDIA_STORAGE_ENABLED === "1";
}
...
async function ensurePublicBucket(settings) {
  if (!mediaStorageEnabled()) {
    throw new Error("Storage de midia em nuvem desativado. Use uma URL publica externa para publicar com midia.");
  }
...
export async function uploadPublicMedia(file) {
  if (!mediaStorageEnabled()) {
    throw new Error("Upload em nuvem desativado para evitar custo de storage. Use uma URL publica externa da midia.");
  }
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    throw new Error("SUPABASE_SERVICE_ROLE_KEY nao configurado no backend.");
  }
...
export async function createSignedUploadUrl({ filename, contentType, size } = {}) {
  ...
  const settings = settingsForContentType(contentType);
  if (!settings || !ALL_TYPES.has(contentType)) throw new Error("Envie apenas JPG, PNG ou MP4.");
  if (Number(size) > settings.maxBytes) {
    const limitMb = Math.floor(settings.maxBytes / 1024 / 1024);
    throw new Error(`${settings.label === "video" ? "Video" : "Imagem"} grande demais. Use ate ${limitMb} MB.`);
  }
  ...
  const folder = settings.label === "video" ? "videos" : "images";
  const objectPath = `${folder}/${datePath}/${randomUUID()}-${slugName(filename)}.${ext}`;

  const signResponse = await fetch(`${supabaseUrl()}/storage/v1/object/upload/sign/${encodePath(`${bucket}/${objectPath}`)}`, {
    method: "POST",
    headers: storageHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({})
  });
  ...
  if (!signResponse.ok) {
    ...
    throw new Error(`Nao foi possivel gerar URL de upload: ${detail}`);
  }

  const signed = await signResponse.json();
  const signedPath = String(signed.url || "").replace(/^\/+/, "");
  if (!signedPath) throw new Error("Supabase nao retornou a URL assinada de upload.");

  return {
    bucket,
    path: objectPath,
    signedUrl: `${supabaseUrl()}/storage/v1/${signedPath}`,
    publicUrl: `${supabaseUrl()}/storage/v1/object/public/${encodePath(`${bucket}/${objectPath}`)}`,
    contentType
  };
}
