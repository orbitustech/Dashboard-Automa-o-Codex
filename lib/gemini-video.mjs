...
function videoPrompt(input = {}) {
  const headline = compact(input.imageText || input.image_text || input.headline, "Coins viram premios").slice(0, 44);
  ...
  return [
    ...
    "Visual story: a person answers a quick survey on the phone, Coins appear only as internal text balance/progress bar, then allowed gift cards appear on screen.",
    `Required compliance rules: ${pesquisaPremiosRulesText()}`,
    "Avoid any coin imagery, coin icons, round currency tokens, chips, medals, physical coins, cash, banknotes, dollar signs, wallets, gold bars, Pix, phone recharge visuals, betting visuals, official brand logos, fake guaranteed rewards, distorted hands, extra fingers, awkward faces and clutter.",
