...
function videoPrompt(input = {}) {
  ...
  const headline = compact(input.imageText || input.image_text || input.headline, compact(input.siteName, "Confira agora")).slice(0, 44);
  const request = compact(input.prompt || input.image_prompt || input.improvementPrompt || input.body);
  const lines = [
    "Vertical 9:16 short social media video ad.",
    "Create a premium, trustworthy, organic ad-style clip with smooth motion, not a hard-sell commercial.",
    `Site context and theme guidance: ${siteContentContext(site)}`,
    `Visual style guidance: ${siteImageStyle(site)}`,
    ...
    "No distorted hands, no extra fingers, no awkward faces, no clutter."
  ];
  if (legacy) {
    lines.push(
      ...
      "Avoid any coin imagery, coin icons, round currency tokens, chips, medals, physical coins, cash, banknotes, dollar signs, wallets, gold bars, Pix, phone recharge visuals, betting visuals, official brand logos, fake guaranteed rewards."
    );
  }
  if (request) lines.push(`Operator request: ${request}`);
  return lines.join(" ");
}
