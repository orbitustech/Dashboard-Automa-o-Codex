...
const DEFAULT_IMAGE_STYLE = "premium clean 3D editorial illustration with one Brazilian adult, gift cards, rewards dashboard, and large headline text, no money imagery, no real brands";
...
function imageHeadline(input) {
  ...
  return "Resgate premios com Koins";
}
...
export async function generateSocialPost(input) {
  const prompt = [
    ...
    "O prompt de imagem deve ser, por padrao, uma ilustracao 3D premium vertical 9:16 com uma pessoa brasileira adulta, smartphone, tela de pontos/resgate, gift cards genericos e uma frase curta grande e legivel.",
...
export function buildImagePrompt(input) {
  ...
  return [
    ...
    "Primary composition: one adult person in a calm trustworthy pose using or holding a smartphone, with a simple rewards dashboard, generic gift cards, recharge/coupon cards, progress/checklist elements, clean premium layout, and clear social-media ad structure.",
