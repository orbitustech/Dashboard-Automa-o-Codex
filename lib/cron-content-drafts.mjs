...
function slotConfig(slot) {
  ...
  return {
      label: "14h",
      titleLabel: "tarde",
      angle: "gancho curioso de meio do dia, mostrando que poucos minutos livres podem virar Coins",
      imageText: "Responda e acumule Coins"
  };
}
...
function generationInput(site, slot, recent) {
  ...
  return {
    ...
    image_prompt: "Criar imagem vertical 9:16 premium para social, com uma pessoa brasileira, smartphone, tela de pontos Coins em texto/progresso e cards de gift card com texto legivel. Nao incluir moedas ou icones de moeda.",
...
export async function createCronContentDraft(slot) {
  ...
  const created = await insertContent({
    ...
    revision_notes: [
      ...
      `Criado automaticamente pelo AWS/EventBridge (${config.label}) em ${new Date().toISOString()}.`,
